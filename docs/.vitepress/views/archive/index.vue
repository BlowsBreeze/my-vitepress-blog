<template>
    <div class="mx-auto my-20 max-w-3xl px-4 pb-8 dark:divide-slate-200/20 sm:px-6 xl:max-w-6xl xl:px-0">
        <!-- 标题 -->
        ...
        <ul class="mt-6 grid grid-cols-1 pt-6 lg:grid-cols-4 lg:gap-8">
            <!-- 所有文章 -->
            <div :class="categories ? 'col-span-3' : 'col-span-4'" class="order-2 pt-6 lg:order-1 lg:mt-0 lg:pt-0">
                <h1
                ...
                <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div v-for="(columnPosts, index) in [firstHalf, secondHalf]" :key="index"
                        class="col-span-1 flex flex-col gap-4">
                        <PostCard v-for="post in columnPosts" :key="post.date.time" :post="post" :flow="flow">
                        </PostCard>
                    </div>
                </div>
            </div>
